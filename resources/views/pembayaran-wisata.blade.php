            ...
            <div class="col-lg-6 py-3 bg-white rounded-3">
                @foreach ($pembayaran as $item)
                    ...
                    <div class="col-12 d-flex justify-content-center pembayaran">
                        ...
                        <h2>Rp.{{ number_format($item->total_reservasi) }}</h2>
                    </div>
                @endforeach
            </div>
            <div class="col-12 mt-2 d-flex justify-content-center">
                <div class="text-center col-8">
                    ...
                    @if ($item->tgl_batas_pembayaran <= Carbon\Carbon::now())
                        <form action="{{ route('cancel-wisata', $item->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            <button type="submit" class="mb-1 col-8 btn btn-danger">Batalkan Pemesanan</button>
                        </form>
                    @else
                        <form action="{{ route('wisataUpload', $item->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
